...
from keyboards import kb_client, kb_ksu_С, kb_ksu_R, kb_son, \
    kb_browser, kb_music, kb_music1, ksu_sites, kb_sc, kb_video, kb_video1, kb_mouse, ksu_back
...
# А может для красоты и емкости сделать рили пульты, когда выучу мобилку? Неплохой pet-project
#
"""
    else:
...
async def load_browser(message: Message, state: FSMContext):
    ...
    if message.text == 'Открыть ссылку':
        ...
        await message.answer("Отправь ссылку: ", reply_markup=ksu_back)
...
async def load_browser2(message: Message):
    ...
    try:
        if message.text == "Назад":
            await message.answer("Упс", reply_markup=kb_browser)
            await Ksu_search.browser.set()
        else:
        ...
        await Ksu_search.browser2.set()
        await message.answer("Что-то пошло не так :(", reply_markup=ksu_back)
...
async def load_sleep(message: Message, state: FSMContext):
    ...
    if message.text == 'Сон':
        ...
        await message.answer("Хорошо. Напиши время через сколько произвести выключение в минутах: ", reply_markup=ksu_back)
...
async def load_sleep2(message: Message):
    try:
        ...
        await message.answer("Братан, используй целые числа")
